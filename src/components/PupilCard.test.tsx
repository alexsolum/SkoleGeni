...
import ClassEditor, { DraggablePupilCard } from "../pages/ClassEditor";
...
beforeEach(() => {
  ...
  store.chemistry = [
    { from_pupil_id: "p1", to_pupil_id: "p3", relationship: "positive" },
    { from_pupil_id: "p1", to_pupil_id: "p2", relationship: "negative" }
  ];
  store.assignment = {
    assignment: [
      ["p1", "p2"],
...
it("renders a red-card pupil state with the violation reason in the tooltip", () => {
  render(
    <DraggablePupilCard
      pupil={{
        id: "p1",
      ...
      violations={[
        {
          ...
          message: "Ada cannot be in the same class as Bea."
        }
      ]}
    />
  );
  ...
  expect(screen.getByText("Ada").closest("[data-red-card='true']")).toHaveAttribute(
    "title",
    "Ada cannot be in the same class as Bea."
  );
});

it("warns when manual edits significantly reduce score categories compared with the optimizer result", async () => {
  render(
  ...
  await waitFor(() => {
    expect(screen.getAllByText("Red Card").length).toBeGreaterThan(0);
  });

  expect(screen.getByLabelText("Score drop warnings")).toHaveTextContent("Chemistry score dropped");
  expect(screen.getByLabelText("Score drop warnings")).toHaveTextContent("Gender score dropped");
});
