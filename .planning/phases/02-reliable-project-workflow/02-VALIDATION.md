...
| 02-02-02 | 02 | 2 | FLOW-04 | integration | `npm run test -- src/pages/__tests__/pupil-autosave.test.tsx` | depends on 02-01 harness and covers blocked-draft restore | ⬜ pending |
...
| Blocked validation edits survive reload as a local draft until corrected | FLOW-04 | The trust model depends on preserving edits even when server autosave is intentionally paused by validation errors | Enter an invalid pupil row, verify `Unsaved validation errors`, reload the page, and confirm the same invalid draft is still present for correction before the next successful save. |
...
- [ ] Blocked-validation persistence is covered by automated tests and manual reload verification
- [ ] No watch-mode flags
